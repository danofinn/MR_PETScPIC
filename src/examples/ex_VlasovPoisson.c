...
PetscErrorCode RHSObjectiveF(TS ts, PetscReal t, Vec U, PetscScalar *F, void *ctx)
{
  AppCtx            *user = (AppCtx *)ctx;
  SNES               snes = ((AppCtx *)ctx)->snes;
  PetscReal         *coords, *pot_int, m_p, q_p;
  PetscReal         *pot;
  PetscScalar       *E;
  DM                 sw, dm;
  const PetscScalar *u;
  PetscInt           dim, d, Np, p, c, cStart, cEnd, fields = 1;
  PetscDS            ds;
  PetscFE            fe;
  PetscFEGeom        feGeometry;
  Vec                phi, locPhi;
  ...
  for (p = 0; p < Np; ++p) {
    ...
    const PetscReal KE  = 0.5 * m_p * v2 + (q_p) * (pot[p]);
    *F += KE;
  }
  PetscCall(VecRestoreArrayRead(U, &u));
  PetscCall(DMSwarmRestoreField(sw, "E_field", NULL, NULL, (void **)&E));
...
PetscErrorCode RHSFunctionG(TS ts, PetscReal t, Vec U, Vec G, void *ctx)
{
  ...
  PetscCall(VecGetArray(G, &g));
  PetscCall(VecGetArrayRead(U, &u));
  PetscCall(VecViewFromOptions(U,NULL,"-U_DG_view"));
  PetscCall(DMSwarmGetField(sw, "E_field", NULL, NULL, (void **)&E));
  PetscCall(DMSwarmGetLocalSize(sw, &Np));
  PetscCall(PetscBagGetData(user->bag, (void **)&param));
  m_p = user->masses[0] * param->m0;
...
/*TEST
   ...
   testset:
     args: -dm_plex_dim 2 -dm_plex_box_bd periodic,none -dm_plex_simplex 0 -dm_plex_box_faces 10,1 -dm_plex_box_lower 0,-0.5 -dm_plex_box_upper 12.5664,0.5\
           -ts_dt 0.03 -ts_max_time 500 -ts_max_steps 500\
     ...
     test:
       ...
       args: -ts_type basicsymplectic -ts_basicsymplectic_type 1 -cosine_coefficients 0.0,0.5 -em_type primal -petscspace_degree 1 -em_pc_type svd
     test:
       suffix: uniform_primal_1d
       args: -ts_type basicsymplectic -ts_basicsymplectic_type 1 -em_type primal -petscspace_degree 1 -em_pc_type svd
     test:
       suffix: uniform_primal_1d_dg
       args: -ts_type discgrad -ts_discgrad_gonzalez -snes_mf_operator -snes_stol 1e-16 -pc_type lu \
             -em_type primal -petscspace_degree 1 -em_pc_type svd \
             -ts_monitor -snes_monitor -snes_converged_reason -snes_linesearch_monitor \
             -ksp_monitor_true_residual -B_view draw -draw_pause 0
     test:
       suffix: uniform_primal_1d_dg_itoh
       args: -ts_type discgrad -ts_discgrad_itoh -snes_mf_operator -snes_stol 1e-16 -pc_type lu \
             -em_type primal -petscspace_degree 1 -em_pc_type svd \
             -ts_monitor -snes_monitor -snes_converged_reason -snes_linesearch_monitor \
             -ksp_monitor_true_residual -B_view draw -draw_pause 0
     test:
       suffix: uniform_none_1d
       args: -ts_type basicsymplectic -ts_basicsymplectic_type 1 -em_type none
     test:
       suffix: uniform_mixed_1d
       args: -ts_type basicsymplectic -ts_basicsymplectic_type 1 -em_type mixed\
             -ksp_rtol 1e-10\
             -em_ksp_type preonly\
